...
def main():
    ...
    for name in ("SUMMARY.md", "comparison_intervals.png", "comparison_intervals.pdf", "RENDER_MANIFEST.json"):
    ...
    lines += [
        ...
        "![Saved Treasury comparison estimates and uncertainty](comparison_intervals.png)", "",
        ...
        "Evidence: [metrics](metrics.json), [independent reconstruction](verification.json), [sample support](support.json), [freeze](freeze_record.json), [selected-test results](TEST_GATE_RESULT.json), [exact test selection](TEST_SELECTION.json), [terminal record](terminal.json), and [PDF figure](comparison_intervals.pdf).",
    ]
    ...
    for suffix in ("png","pdf"):
        fig.savefig(REPORT/f"comparison_intervals.{suffix}",dpi=180,facecolor="white")
    plt.close(fig)
    manifest = {"status":"RENDERED_FROM_SAVED_VERIFIED_METRICS_ONLY","metrics_sha256":sha(REPORT/"metrics.json"),
                "renderer_sha256":sha(Path(__file__)),"artifacts":{n:sha(REPORT/n) for n in ("SUMMARY.md","comparison_intervals.png","comparison_intervals.pdf")},
