...
from src.treasury_history_identity import UnsupportedIdentity, reconcile_history_identity
...
def main():
    ...
    cp_path = rep / "TREASURY_HISTORY_IDENTITY_CHECKPOINT_V3.json"
    ...
    previous_identity_hash = hashlib.sha256(
        required("reports/next_signal_review/TREASURY_HISTORY_IDENTITY.json")
    ).hexdigest()
    ...
    output = rep / "TREASURY_HISTORY_IDENTITY_V2.json"
    if output.exists():
    ...
    for selected in selection["records"]:
        ...
        try:
            ...
            identity = reconcile_history_identity(
                a_raw,
                a_text,
                r_raw,
                r_text,
