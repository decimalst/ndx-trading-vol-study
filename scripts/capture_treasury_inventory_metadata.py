...
PRIVATE = ROOT / "data/source_discovery/treasury_auction/inventory_capture_v2"
...
def run():
    checkpoint = REPORTS / "TREASURY_INVENTORY_CHECKPOINT.json"
    ...
    if (REPORTS / "TREASURY_INVENTORY_CAPTURE.json").exists():
    ...
    for start, end, url in annual_requests():
        ...
        if start == "2010-01-01":
            legacy = ROOT / "data/source_discovery/treasury_auction/archive_capture_v1/2010"
            for name in [
                "response.json",
                "headers.txt",
                "receipt.json",
                "transport_stderr.txt",
            ]:
                with (year_dir / name).open("xb") as out:
                    out.write((legacy / name).read_bytes())
            body = body_path.read_bytes()
            receipt = json.loads((year_dir / "receipt.json").read_text())
        else:
            proc = subprocess.run(
                [
                    "curl",
        ...
        entry = {
            "year": start[:4],
            "capture_mode": "reused_original_bytes"
            if start == "2010-01-01"
            else "new_http_request",
            "start_date": start,
            "end_date": end,
    ...
    write_json(REPORTS / "TREASURY_INVENTORY_CAPTURE.json", finished)
