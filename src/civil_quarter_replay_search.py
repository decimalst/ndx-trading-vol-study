"""Source-complete replay of the unscored civil risk question; full failed-trial accounting."""
...
from .verify_civil_quarter_replay import validate_upstream

ROOT = Path(__file__).resolve().parents[1]
PROTOCOL = ROOT / "civil_quarter_replay.yaml"
...
def validate(p):
    if p != CONTRACT:
        raise ValueError("Fixed civil-quarter-replay protocol differs")
...
def inherited(p, pins=None):
    ...
    for source_name in p["comparisons"]["inherited_sources"]:
        ...
        if pins is not None and pins.get(source_name) != signature:
            raise ValueError("Pinned inherited metrics changed before decoding: " + source_name)
        for number, row in enumerate(json.loads(payload)["rows"]):
            rows.append(
                {
                    "study": row.get("study", Path(source_name).parent.name or Path(source_name).stem),
    ...
    if len(rows) != 123:
        raise ValueError("All123 inherited comparisons required")
    return rows
...
def evaluate(panel, features, p, monthly_fits, *, prior=None):
    ...
    if len(prior) != 123:
        raise ValueError("Complete123 inherited family required")
...
def report(metrics):
    lines = [
        "# SPX civil quarter-end risk experiment with complete source staging",
        "",
...
def input_paths(p):
    ...
    for anchor in ("upstream", "feature_source", "failed_attempt"):
        info = p[anchor]
...
def run():
    ...
    if (REPORT / "manifest.json").exists():
        raise ValueError("Refusing to overwrite a registered civil-quarter-replay experiment")
    modules = [
        "tests.test_civil_quarter_sources",
        "tests.test_civil_quarter_replay_search",
        "tests.test_civil_quarter_replay_publication",
        "tests.test_civil_quarter_replay_integration",
        "tests.test_verify_civil_quarter_replay",
        "tests.test_plot_civil_quarter_replay",
    ...
    if checked.returncode:
        raise RuntimeError(
            "Prewritten civil-quarter-replay tests failed; no registration or fit"
        )
    ...
    try:
        ...
        if inference.digest(PROTOCOL) != manifest["protocol_sha256"]:
            raise ValueError("Frozen protocol changed during civil-quarter-replay run")
