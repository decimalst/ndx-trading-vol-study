...
def validated_rows(metrics, verification, protocol_hash):
    ...
    if (
        ...
        raise ValueError("Exact registered two-comparison and cumulative127 family required")
...
def render_verified(metrics, verification, protocol_hash, folder):
    ...
    try:
        ...
        fig.text(
            ...
            f"Scales differ. Both controls require effect/stability gates, wave Holm2 p < {WAVE_ALPHA:.9g} and cumulative Holm127 p < 0.05.\n"
...
def main(root=ROOT):
    ...
    protocol_hash = hashlib.sha256((root / "profiled_quarter.yaml").read_bytes()).hexdigest()
