...
def validated_rows(metrics, verification, protocol_hash):
    ...
    if (
        ...
        or metrics.get("cumulative_hypothesis_count") != 123
    ):
        raise ValueError("Exact registered two-comparison and cumulative123 family required")
...
def render_verified(metrics, verification, protocol_hash, folder):
    ...
    try:
        ...
        fig.suptitle(
            "Does a civil quarter-end increment improve next-session SPX risk forecasts?",
            x=0.06,
            ha="left",
            fontsize=13,
        )
...
def main(root=ROOT):
    root = Path(root)
    folder = root / "reports/civil_quarter"
    ...
    protocol_hash = hashlib.sha256((root / "civil_quarter.yaml").read_bytes()).hexdigest()
