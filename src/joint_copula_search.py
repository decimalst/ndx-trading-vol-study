"""One registered joint-return dependence experiment; no silent retry or fallback."""
...
from src.joint_copula_protocol import (
    EVIDENCE_CLASS,
    EVIDENCE_LIMITATION,
    EXECUTION,
    PROTOCOL_SHA256,
...
def _invalidate(report, signature, prior, error):
    from src.joint_copula_score import failure_metrics
...
def execute_registered(
    ...
    from src.joint_copula_score import _prior, _probabilities
    ...
    if any(
        ...
        raise ValueError("Refusing to overwrite or restart registered joint-copula attempt")
    _events_bound(
        report / "trial_ledger.jsonl",
        [
            {
    ...
    try:
        ...
        if (
            ...
            or result["hypothesis_count"] != 2
            or result["cumulative_hypothesis_count"] != 151
            or [
                (
                    r.get("study"),
                    r.get("candidate"),
            ...
            != [
                ("joint_copula", "t8_copula", c, 1, "negative_joint_log_density")
                for c in CONTROLS
            ]
        ):
            raise ValueError("Complete ordered registered two-comparison family required")
...
def _source_pins(root):
    ...
    from src.joint_copula_score import inherit_family

    root = Path(root)
    pins = {**EXECUTION["market_pins"], **EXECUTION["proposal_pins"]}
    for key in PIN_KEYS:
...
def _verify_freeze(root, protocol, frozen):
    ...
    validate(protocol)
    if (
        frozen["status"] != "FROZEN_BEFORE_JOINT_COPULA_MARKET_COHORT_OR_FITS"
        or frozen["protocol_sha256"] != PROTOCOL_SHA256
        or frozen["execution"] != EXECUTION
    ):
        raise ValueError("Exact prospective joint-copula execution freeze required")
    current = _code(root)
    if "src/joint_copula_search.py" not in current or set(frozen["code"]) != set(current):
        raise ValueError("Complete current source/test inventory required")
    required_inputs = {
        **EXECUTION["market_pins"],
        **EXECUTION["proposal_pins"],
...
def freeze(root=ROOT):
    ...
    if not (root / "reports/joint_copula/prefit/INDEPENDENT_EXECUTION_REVIEW.md").is_file():
    ...
    record = {
        ...
        "cumulative_hypotheses_before_registration": 149,
        "previous_goal_turn": "NO_PROGRESS_STATUS_RECHECK_OF_COMPLETED_WAVE26;_REVALIDATED_NEXT_SAFE_ACTION_NEW_STUDY",
        "environment": {
            "python": sys.version,
            "packages": {
                n: version(n)
                for n in ("numpy", "pandas", "scipy", "pyarrow", "PyYAML", "mpmath")
            },
            "threads": {
                n: os.environ.get(n)
...
def run(root=ROOT):
    ...
    from src.joint_copula_pipeline import PipelineExecutionError, produce
    ...
    def save_produced(produced):
        for name in ("features", "targets", "applications", "panel", "coverage", "schedules"):
    ...
    def work():
        print(
            "Two comparisons registered; decoding bounded admitted market inputs", flush=True
        )
        ...
        qqq, spx, iv, source_audit = read_sources(root, EXECUTION["market_pins"])
        save_report("source_admission.json", source_audit)
        print(
            "Issuing monthly shared marginals and certified dependence forecasts", flush=True
        )
        try:
            produced = produce(
                qqq,
                spx,
                iv,
                pipeline_config(protocol),
                measurement_callback=lambda audit: save_report(
                    "measurement_audit.json", audit
                ),
            )
        ...
        _check_output_snapshot(root, output_pins)
        # Verify the actual serialized outputs, as well as their initial hashes.
        import pandas as pd

        produced = {
            name: pd.read_parquet(out / (name + ".parquet"))
            for name in (
                "features",
                "targets",
                "applications",
        ...
        produced["fits"] = json.loads((out / "fits.json").read_bytes())
        features = produced["features"]
        print(
            "Independently reconstructing source features, marginal fits and global dependence certificates",
            flush=True,
        )
        forecast_check = verify_forecasts(qqq, spx, iv, produced, pipeline_config(protocol))
        ...
        print(
            "Scoring both comparisons and independently reconstructing inference",
            flush=True,
        )
    ...
    try:
        ...
        save_report("terminal.json", terminal())
    print(
        json.dumps({"status": result["status"], "leads": result["leads"], "comparisons": 151}),
        flush=True,
    )
    return result
