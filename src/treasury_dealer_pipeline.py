"""Fixed monthly treasury forecasts on one unchanged observed calendar."""
...
def _configuration(config):
    ...
    if type(config) is not dict or set(config) != fields:
        raise ValueError("Exact treasury pipeline configuration fields required")
    parsed = {
...
def _inputs(features, targets, config):
    required = set(ALL) | {"treasury_cutoff_date"}
    if (
    ...
    cutoffs = _dates(features["treasury_cutoff_date"], config["source_end"])
    ...
    if not cutoffs.equals(expected_cutoffs):
        raise ValueError(
            "Treasury cutoff must be the prior full-calendar session after the source floor"
        )
    if np.any(complete & cutoffs.isna()):
        raise ValueError("Complete common32 features require known treasury cutoff metadata")
        ...
        rows.append(
            {
                ...
                "treasury_cutoff_date": cutoffs[position],
...
# Public spelling retained for callers that name the monthly operation a run.
run_pipeline = build_panel
