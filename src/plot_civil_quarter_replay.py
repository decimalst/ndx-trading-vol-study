"""Verified proper-variance comparisons for one civil quarter-end increment."""
...
def validated_rows(metrics, verification, protocol_hash):
    """Require current verification and exactly two same-sample proper-variance contrasts."""
    if (
    ...
    if (
        ...
        raise ValueError("Exact registered two-comparison and cumulative125 family required")
    ...
    proof_fields = {
        "forecasts_verified": "new_forecasts",
        "monthly_fits_verified": "new_monthly_fits",
    ...
    if (
        ...
        raise ValueError(
            "Complete three-model cohort and independently verified fit/application counts required"
        )
    rows = metrics.get("rows", [])
    keys = [(r.get("candidate"), r.get("control"), r.get("score")) for r in rows]
    if len(keys) != 2 or set(keys) != set(CONTRASTS):
        raise ValueError("Both distinct registered proper-variance comparisons required")
    result, cohort = [], None
    for contrast in CONTRASTS:
        ...
        for phase in ordered:
            ...
            if (
                ...
                raise ValueError(
                    "Finite proper-loss differences and literal ordered intervals required"
                )
...
def render_verified(metrics, verification, protocol_hash, folder):
    """Render literal natural-log-score intervals without percent rescaling."""
    ...
    try:
        for ax, row in zip(axes, rows, strict=True):
            ...
            control = "full baseline" if row["control"] == "baseline" else "historical mean"
            ax.set_title(f"Quarter-end vs {control}", loc="left", fontsize=11, y=1.13)
            ax.text(
                0.0,
                1.025,
            ...
            ax.set_xlabel(
                "Candidate − control proper variance loss\n(natural-log-score units)",
                fontsize=9,
            )
        ...
        fig.suptitle(
            "Replay: does a civil quarter-end increment improve SPX risk forecasts?",
            x=0.06,
            ha="left",
            fontsize=13,
        )
        fig.text(
            0.06,
            0.035,
            "Lines: nominal 95% block-bootstrap/HAC interval envelopes. Negative is better; dotted reference: −0.005.\n"
            f"Scales differ. Both controls require effect/stability gates, wave Holm2 p < {WAVE_ALPHA:.9g} and cumulative Holm125 p < 0.05.\n"
            "Archival daily OHLC risk proxy; no exchange-expiry, rebalance or trading-profit claim.",
            fontsize=8.5,
            color="#555555",
        )
...
def main(root=ROOT):
    root = Path(root)
    folder = root / "reports/civil_quarter_replay"
    ...
    protocol_hash = hashlib.sha256(
        (root / "civil_quarter_replay.yaml").read_bytes()
    ).hexdigest()
