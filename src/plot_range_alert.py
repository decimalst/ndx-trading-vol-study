"""Verified Brier comparisons for one closing-location increment."""
...
def validated_rows(metrics, verification, protocol_hash):
    ...
    if (
        ...
        raise ValueError("Exact registered two-comparison and cumulative129 family required")
...
def render_verified(metrics, verification, protocol_hash, folder):
    ...
    try:
        for ax, row in zip(axes, rows, strict=True):
            ...
            ax.set_title(f"Location vs {control}", loc="left", fontsize=11, y=1.13)
            ax.text(
                0.0,
                1.025,
        ...
        fig.suptitle(
            "Does closing location help predict unusually high SPX risk?",
            x=0.06,
            ha="left",
            fontsize=13,
        )
        fig.text(
            ...
            f"Scales differ. Both controls require effect/stability gates, wave Holm2 p < {WAVE_ALPHA:.9g} and cumulative Holm129 p < 0.05.\n"
...
def main(root=ROOT):
    ...
    protocol_hash = hashlib.sha256((root / "range_alert.yaml").read_bytes()).hexdigest()
