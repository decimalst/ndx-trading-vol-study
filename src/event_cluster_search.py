...
from . import event_cluster_admission as admission
...
CONTRACT = {
    ...
    "objective": "Test within-series adjacent mature risk events beyond fixed market forecast and matched "
    "event count/recency controls",
    "original_index": {
        "asset": "SPX",
    ...
    "history": {
        ...
        "unissued/unscored/feature-incomplete origins; not historical forecast errors",
        "unknown": "every original training/application window must contain22 known labels; "
        "otherwise whole-trial failure before fitting; never fill, compress or drop "
        "original rows",
        "nuisance": [
            "event_fraction22",
            "event_fraction22_sq",
        ...
        "diagnostic": "excess_adjacency22=(21*C-(N-last_event)*(N-1))/441; never enters fit or "
        "promotion",
        "arithmetic": "exact Python integer numerators followed by single fixed division; N*N/484 "
        "is not square of rounded N/22",
        "centering": "training-only math.fsum/n; exactconstant uses firstvalue; fixedscale1 and no "
        "rank/column deletion",
    },
    ...
    "model": {
        ...
        "scalar_arithmetic": "finitefloat64; signedsoftplus and signedresidual remain nonzero for "
        "finite logits; checked nonzero products rejectunderflow; compensated "
        "scalar reductions",
        "nuisance_arithmetic": "frozen logistic_state numerical contract; finite objective, "
        "fullgradient and Hessian; no optional unusedcurvature admission gate",
    ...
    "inference": {
        ...
        "seed": 20260926,
        "seed_rule": "seed+phase_code*10000+block; development0,evaluation1; same drawdesign "
        "acrosscontrols",
        "wave_alpha": 0.00011904761904761905,
        "cumulative_alpha": 0.05,
        "p_phase": "maximum two-sided centered-null circularblock bootstrap and BartlettHAC126p",
        ...
        "calibration": "descriptivein-the-large meanprob,eventrate,gap,Brier perphase/model; no "
        "fittedcalibrationbins",
    ...
    "upstream": {
        ...
        "andzero-hypothesisearlysessiondocumentation; checkedbytes beforedecode",
    },
...
def inherited(p, pins=None):
    ...
    if len(rows) != 131:
        raise ValueError("All131 inherited comparisons required")
    if any(
...
def evaluate(panel, calendar, p, monthly_fits, *, prior=None):
    ...
    return {
        ...
        "new_forecasts": int(panel.model.isin(["nuisance", "cluster"]).sum()),
        "reused_control_forecasts": int(
...
def input_paths(p):
    ...
    paths.add("reports/event_cluster/freeze_record.json")
...
def run():
    ...
    modules = [
        "tests.test_event_cluster_admission",
        "tests.test_event_cluster_features",
        "tests.test_event_cluster_models",
        "tests.test_event_cluster_pipeline",
        "tests.test_event_cluster_verification",
        "tests.test_event_cluster_search",
        "tests.test_verify_event_cluster",
        "tests.test_event_cluster_publication",
        "tests.test_plot_event_cluster",
    ]
    ...
    try:
        ...
        panel, fits, states, memory, support = sm.forecast_panel(
            loaded["features"],
            loaded["targets"],
            loaded["forecasts"],
            loaded["fits"],
        ...
        memory.to_parquet(OUT / "memory.parquet")
        ...
        if any(
            ...
            for name in ("range_alert", "issued_calibration")
        ):
