"""Frozen QQQ-SPX joint intraday return risk study and complete trial accounting."""
...
from . import joint_risk_features as cf
from . import joint_risk_models as cm
from . import orthogonal_round2 as inference
from .international_search import diagnostics

ROOT = Path(__file__).resolve().parents[1]
PROTOCOL = ROOT / "joint_risk.yaml"
REPORT = ROOT / "reports/joint_risk"
OUT = ROOT / "data/joint_risk"
CONTRASTS = (
...
def validate(p):
    ...
    if (
        any(p["verification"][k] != v for k, v in fixed_verification.items())
        or any(i[k] != v for k, v in expected.items())
        or any(inf[k] != v for k, v in fixed_inf.items())
        or p["wave"] != 10
        or p["wave_alpha"] != WAVE_ALPHA
        or c["new_hypotheses"] != 2
        or c["inherited_hypotheses"] != 110
        or c["cumulative_hypotheses"] != 112
        ...
        raise ValueError("Fixed joint risk replication specification differs")
...
def paired_inference(candidate, control, p, seed):
    ...
    if (
        ...
        raise ValueError("Finite aligned joint matrix scores required")
    d = candidate - control
...
def inherited(p):
    ...
    if len(rows) != 110:
        raise ValueError("All110 inherited comparisons required")
    return rows
...
def passes(row):
    ...
    return (
        ...
        and all(x["n"] > 0 and x["delta"] <= -0.005 for x in phases)
...
def evaluate(panel, calendar, p):
    ...
    for candidate, control, score in CONTRASTS:
        ...
        for code, name in enumerate(["development", "evaluation"]):
            ...
            wide = frame.pivot(index="origin", columns="model", values="loss").sort_index()
            if set(wide.columns) != set(cf.MODELS) or not np.isfinite(wide).all().all():
                raise ValueError("Complete common finite signed joint-risk forecasts required")
            models = {
            ...
            if not np.isfinite(actual[["y_qqq", "y_spx"]]).all().all():
                raise ValueError("Finite signed paired return labels required")
            for column in ["h_qqq", "h_spx"]:
                if not models["constant_correlation"][column].equals(
                    models["dynamic_correlation"][column]
                ):
                    raise ValueError(
                        "Conditional dependence models must share exact diagonals"
                    )
            losses = {}
            for model, other in models.items():
                e = (
                    other[["y_qqq", "y_spx"]].to_numpy()
                    - other[["mu_qqq", "mu_spx"]].to_numpy()
                )
                losses[model] = cm.density.matrix_score(
                    e, other[["h_qqq", "h_spx"]].to_numpy(), other.rho.to_numpy()
                )
                if not np.allclose(losses[model], other.loss, rtol=1e-10, atol=1e-10):
                    raise ValueError("Saved joint score disagrees with forecast matrix")
            loss1, loss0 = losses[candidate], losses[control]
...
def report(metrics):
    lines = [
        "# QQQ-SPX joint intraday return risk",
        "",
    ...
    lines += [
        ...
        "",
        "One dependence slope with shared modeled marginals; archival ETF/index daily return proxy and historical reuse remain exploratory. A gain need not identify true correlation dynamics if shared marginals are misspecified.",
        "",
    ]
...
def run():
    ...
    if (REPORT / "manifest.json").exists():
        raise ValueError("Refusing to overwrite a registered joint-risk experiment")
    modules = [
        "tests.test_joint_risk_features",
        "tests.test_joint_risk_density",
        "tests.test_joint_risk_models",
        "tests.test_joint_risk_search",
        "tests.test_joint_risk_publication",
        "tests.test_verify_joint_risk",
    ...
    if checked.returncode:
        raise RuntimeError("Prewritten joint-risk tests failed; no registration or fit")
    ...
    try:
        ...
        if inference.digest(PROTOCOL) != manifest["protocol_sha256"]:
            raise ValueError("Frozen protocol changed during joint-risk run")
