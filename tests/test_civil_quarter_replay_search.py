...
from src import civil_quarter_replay_search as study
...
class CivilQuarterReplaySearch(unittest.TestCase):
    def test_fixed_contract_and_family(self):
        study.validate(protocol())
        self.assertEqual(study.WAVE_ALPHA, 0.05 / (16 * 17))
        self.assertEqual(
            study.CONTRASTS,
            (
        ...
        self.assertEqual(len(study.inherited(protocol())), 123)
    ...
    def test_model_and_evidence_geometry_are_exact_failed_attempt(self):
        ...
        self.assertEqual(new["inference"]["seed"], 20260922)
    ...
    def test_all_unevaluable_hypotheses_retained(self):
        ...
        self.assertEqual((m["hypothesis_count"], m["cumulative_hypothesis_count"]), (2, 125))
