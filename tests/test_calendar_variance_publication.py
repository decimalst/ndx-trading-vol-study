"""Pre-fit publication failures preserve the whole two-contrast calendar family."""
import hashlib
import json
import tempfile
import unittest
...
from src import calendar_variance_search as study


class TestCalendarVariancePublication(unittest.TestCase):
    def _assert_failed_publication(self, fault):
        ...
        scored = {
            "rows": [
                {"study": "calendar_variance", "candidate": candidate,
        ...
        with tempfile.TemporaryDirectory() as folder, ExitStack() as stack:
            ...
            protocol.write_text(json.dumps({"sources": {
                ...
                "vvix": "vvix.csv", "cpi": "cpi.json", "nfp": "nfp.json",
                "fomc": "fomc.csv", "fomc_coverage": "fomc_coverage.json"}, "index": {}}))
            ...
            stack.enter_context(patch.object(study.cf, "build_features", return_value=(
                pd.DataFrame(), pd.DataFrame())))
            stack.enter_context(patch.object(study.cm, "forecast_panel", return_value=(
            ...
            self.assertEqual((failed["hypothesis_count"], failed["cumulative_hypothesis_count"]), (2, 108))
            ...
            self.assertEqual(sum(r["event"] == "inherited" for r in events), 106)
