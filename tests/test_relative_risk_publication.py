"""Pre-fit publication failures preserve the whole two-contrast relative-risk family."""
import hashlib
import json
import tempfile
import unittest
...
class TestRelativeRiskPublication(unittest.TestCase):
    def _assert_failed_publication(self, fault):
        ...
        scored = {
            "rows": [
                {"study": "relative_risk", "candidate": candidate,
                 "control": control, "score": score, "horizon": 1,
        ...
        measurement_audit = {"synthetic": True, "status": "PASS", "diagnostic": "preserve measurement audit"}
        ...
        with tempfile.TemporaryDirectory() as folder, ExitStack() as stack:
            ...
            protocol.write_text(json.dumps({"sources": {
                "qqq": "qqq.parquet", "daily": "spx.parquet", "vxn": "vxn.csv",
                "vix": "vix.csv", "vix9d": "vix9d.csv", "vvix": "vvix.csv"}, "index": {}}))
            ...
            stack.enter_context(patch.object(study.cf, "load_sources", return_value=(None, None, None, source_audit)))
            stack.enter_context(patch.object(study.cf, "measurement_audit", return_value=measurement_audit))
            stack.enter_context(patch.object(study.cf, "require_measurement", side_effect=require_measurement))
            build = stack.enter_context(patch.object(study.cf, "build_features", return_value=(
                pd.DataFrame(), pd.DataFrame())))
            fit = stack.enter_context(patch.object(study.cm, "forecast_panel", return_value=(
                pd.DataFrame({"synthetic": [1]}), fit_audit)))
            stack.enter_context(patch.object(pd.DataFrame, "to_parquet", new=save_synthetic_parquet))
            expected_error = ValueError if fault in {"protocol_mutation", "measurement"} else OSError
            expected_message = "[Pp]rotocol" if fault == "protocol_mutation" else ("INSUFFICIENT_DATA" if fault == "measurement" else "Injected")
            ...
            self.assertEqual((failed["hypothesis_count"], failed["cumulative_hypothesis_count"]), (2, 110))
            self.assertEqual(len(failed["rows"]), 2)
            self.assertEqual({(r["candidate"], r["control"], r["score"]) for r in failed["rows"]},
                             set(study.CONTRASTS))
            expected_status = "INSUFFICIENT_DATA" if fault == "measurement" else "INVALID_RUN"
            self.assertTrue(all(r["p_conservative"] == 1 and r["status"] == expected_status
                                for r in failed["rows"]))
            self.assertTrue(all(r["p_holm_wave"] == 1 and r["p_holm_cumulative"] == 1
                                for r in failed["rows"]))
            ...
            if fault in {"registered_ledger", "measurement"}:
                ...
                if fault == "measurement":
                    self.assertEqual(json.loads((output/"measurement_audit.json").read_text()), measurement_audit)
            ...
            self.assertEqual(sum(r["event"] == "inherited" for r in events), 108)
            self.assertEqual(sum(r["event"] == "registered" for r in events), 1 if fault == "registered_ledger" else 2)
            self.assertEqual(sum(r["event"] == "evaluated" for r in events), int(fault == "evaluated_ledger"))
            self.assertTrue(all(r["event"] == "unevaluable" and r["p_conservative"] == 1 for r in events[-2:]))
            self.assertEqual({(r["candidate"], r["control"], r["score"]) for r in events[-2:]},
                             set(study.CONTRASTS))
    ...
    def test_partial_evaluated_ledger_failure_invalidates_all_two_and_preserves_diagnostics(self):
