...
from src import plot_event_cluster_replay as plot
...
class PlotContracts(unittest.TestCase):
    def setup_files(self, folder):
        ...
        proof = {
            ...
            "verified_output_hashes": {
                "reports/event_cluster_replay/metrics.json": hashlib.sha256(
                    payload
                ).hexdigest()
            },
        }
        ...
        old.parent.mkdir()
        old.write_bytes(b'{"status":"UNEVALUABLE"}')
        (report / "manifest.json").write_text(
            json.dumps(
                {
                    "inputs": {
                        "reports/event_cluster/failure.json": hashlib.sha256(
                            old.read_bytes()
                        ).hexdigest()
                    }
                }
    ...
    def test_old_failure_is_bound_and_preserved(self):
        with tempfile.TemporaryDirectory() as folder:
            root, _, _ = self.setup_files(folder)
            self.assertEqual(plot.load_verified(root), metrics())
            old = root / "reports/event_cluster/failure.json"
            old.write_bytes(b"changed failure record")
            with self.assertRaises(ValueError):
                plot.load_verified(root)
