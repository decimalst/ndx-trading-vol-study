...
class CausalPoolPublication(unittest.TestCase):
    ...
    def test_input_bytes_checked_before_decode_and_one_snapshot(self):
        with tempfile.TemporaryDirectory() as tmp, patch.object(study, "ROOT", Path(tmp)):
            ...
            (root / "fits.json").write_text('[{"seed":0.5}]')
            p = {
                "upstream": {
                    **dict.fromkeys(["features", "targets", "forecasts"], "one.parquet"),
                    "fits": "fits.json",
                }
            }
            pins = {
                name: hashlib.sha256((root / name).read_bytes()).hexdigest()
                for name in ["one.parquet", "fits.json"]
            }
            original = pd.read_parquet

            def decode(snapshot, *args, **kwargs):
                self.assertIsInstance(snapshot, io.BytesIO)
                (root / "one.parquet").write_bytes(b"changed after read")
                return original(snapshot, *args, **kwargs)

            # All logical inputs are snapshotted and checked before the first decoder.
            with patch.object(study.pd, "read_parquet", side_effect=decode):
                frames = study.load_pinned_inputs(p, {"inputs": pins})
            self.assertEqual([f.iloc[0, 0] for f in frames[:3]], [1.0] * 3)
            self.assertEqual(frames[3], [{"seed": 0.5}])
            with (
                patch.object(study.pd, "read_parquet") as decoder,
                self.assertRaisesRegex(ValueError, "Pinned upstream input changed"),
            ):
                study.load_pinned_inputs(p, {"inputs": pins})
    ...
    def test_inherited_json_pin_is_checked_before_decode(self):
        with tempfile.TemporaryDirectory() as tmp, patch.object(study, "ROOT", Path(tmp)):
            ...
            rows = [
                {"candidate": str(i), "horizon": 1, "p_conservative": 0.5} for i in range(119)
            ]
    ...
    def exercise(self, fault):
        ...
        with tempfile.TemporaryDirectory() as tmp, ExitStack() as stack:
            ...
            output = root / "data"
            p = root / "new.yaml"
            p.write_text(json.dumps({"upstream": {"features": "raw.bin"}, "index": {}}))
            ...
            for k, v in {
                ...
                "application_origins": lambda *a: pd.Index([0]),
                "evaluate": score,
                "report": render,
                "ledger": record,
            ...
            self.assertEqual(sum(r["event"] == "inherited" for r in events), 119)
