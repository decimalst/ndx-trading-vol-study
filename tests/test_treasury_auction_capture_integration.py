...
from scripts import capture_treasury_archive_metadata as capture
...
class CaptureIntegrationTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.reports / "TREASURY_ARCHIVE_CHECKPOINT.json").write_text(
    ...
    def test_complete_capture_then_refuses_replay(self):
        with patch.object(capture.subprocess, "run", side_effect=self.fake_http) as net:
            ...
            self.assertEqual(net.call_count, 16)
            result = json.loads((self.reports / "TREASURY_ARCHIVE_CAPTURE.json").read_text())
    ...
    def test_transport_failure_is_saved_and_stops_without_retry(self):
        ...
        self.assertTrue((self.private / "2010/receipt.json").is_file())
        self.assertFalse((self.private / "2010/metadata.json").exists())
