...
class PlotContracts(unittest.TestCase):
    def test_replay_threshold_and_cumulative_family_are_not_previous_wave(self):
        self.assertEqual(plot.WAVE_ALPHA, 0.05 / (16 * 17))
        self.assertEqual(plot.CUMULATIVE_FAMILY, 125)
        protocol = yaml.safe_load((plot.ROOT / "civil_quarter_replay.yaml").read_bytes())
        ...
        with (
            ...
            self.assertIn("Holm125", text)
            self.assertIn("Replay", text)
        metrics["cumulative_hypothesis_count"] = 123
    ...
    def test_invalid_input_creates_no_artifacts_and_main_failure_blocks_rendering(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root / "civil_quarter_replay.yaml").write_bytes(payload)
