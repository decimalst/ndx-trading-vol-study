"""Prospective invented-data tests for fixed treasury monthly scheduling."""
...
from src.treasury_dealer_models import ALL, fit_models
from src.treasury_dealer_pipeline import InsufficientDataError, build_panel, run_pipeline
...
def synthetic_inputs(calendar=None):
    ...
    features["treasury_cutoff_date"] = cutoff.where(
        cutoff >= pd.Timestamp("2010-01-01")
    ).astype("datetime64[ns]")
    features.loc[features.treasury_cutoff_date.isna(), "prior_share_mean_sum"] = np.nan
...
class TreasuryDealerPipelineTests(unittest.TestCase):
    def test_run_pipeline_alias_and_treasury_only_cutoff_schema(self):
        self.assertIs(run_pipeline, build_panel)
        ...
        for name in ("applications", "panel", "coverage"):
            self.assertIn("treasury_cutoff_date", result[name].columns)
            self.assertNotIn("commodity_cutoff_date", result[name].columns)

    def run_mock(self, features, targets, config):
        with patch(
            "src.treasury_dealer_pipeline.fit_models", side_effect=mock_models
        ) as model:
    ...
    def test_earliest_feature_complete_query_is_label_blind(self):
        ...
        features.loc[january[0], "dealer_surprise"] = np.nan
    ...
    def test_common_training_rows_and_strict_previous_session_maturity(self):
        ...
        features.loc[hole, "dealer_surprise"] = np.nan
        result, model = self.run_mock(features, targets, config)
    ...
    def test_all_requested_origins_and_full_calendar_offsets_survive_holes(self):
        ...
        features.loc[holes, ["tlt_lrv22", "prior_share_mean_sum"]] = np.nan
        result, _ = self.run_mock(features, targets, config)
        coverage = result["coverage"].set_index("origin")
        self.assertTrue(coverage.index.equals(requested))
        self.assertTrue(
            (coverage.loc[holes, "missing_features"] == "tlt_lrv22|prior_share_mean_sum").all()
        )
    ...
    def test_unscored_applications_and_censoring_precedence(self):
        ...
        features.loc["2016-01-12", "prior_share_mean_sum"] = np.nan
        result, _ = self.run_mock(features, targets, config)
        coverage = result["coverage"].set_index("origin")
        expected = {
    ...
    def test_empty_requested_and_all_incomplete_months_are_explicit(self):
        ...
        features.loc["2016-01-01":"2016-01-31", "dealer_surprise"] = np.nan
        result, model = self.run_mock(features, targets, config)
    ...
    def test_insufficient_month_aborts_attempt_with_full_coverage(self):
        ...
        features.loc["2016-01-04", "prior_share_mean_sum"] = np.nan
        with (
            patch("src.treasury_dealer_pipeline.fit_models", side_effect=mock_models) as model,
    ...
    def test_unscored_forecast_validation_and_model_failure_are_fatal(self):
        ...
        for invalid in (0.0, -1.0, np.inf, np.nan):
            ...
            with (
                ...
                patch("src.treasury_dealer_pipeline.fit_models", side_effect=invalid_last),
    ...
    def test_qlike_matches_literal_formula_and_overflow_aborts(self):
        ...
        with (
            patch("src.treasury_dealer_pipeline.fit_models", side_effect=tiny),
            self.assertRaises(ValueError),
        ):
            build_panel(features, targets, config)

    def test_exact_target_endpoint_and_prior_full_calendar_treasury_cutoff(self):
        features, targets, config = synthetic_inputs()
        for name in ("target_end", "treasury_cutoff_date"):
        ...
        f.loc["2016-01-08", "treasury_cutoff_date"] = pd.NaT
        with self.assertRaises(ValueError):
            self.run_mock(f, targets, config)

    def test_pre_source_floor_cutoff_is_unknown_and_not_complete(self):
        calendar = pd.bdate_range("2009-12-28", "2010-04-09").as_unit("ns")
        features, targets, config = synthetic_inputs(calendar)
        config.update(
            origin_start="2010-01-01",
            origin_end="2010-04-09",
            development=["2010-01-01", "2010-02-26"],
            evaluation=["2010-03-01", "2010-04-09"],
            source_end="2010-04-09",
            minimum_train=1,
        )
        # Unknown treasury cutoff cannot masquerade as a fully observed row.
        features.loc["2010-01-01", "prior_share_mean_sum"] = 0.5
        with self.assertRaises(ValueError):
    ...
    def test_numeric_and_calendar_validation(self):
        features, targets, config = synthetic_inputs()
        for field, value in (
            ("prior_share_mean_sum", np.inf),
            ("const", 2.0),
            ("prior_share_mean_sum", True),
            ("prior_share_mean_sum", "3"),
        ):
            f = features.copy(deep=True)
    ...
    def test_config_defaults_and_bounded_exact_configuration(self):
        features, targets, config = synthetic_inputs()
        with patch("src.treasury_dealer_pipeline.fit_models", side_effect=mock_models):
    ...
    def test_native_datetime_unit_transport_and_stable_output_schema(self):
        ...
        f["treasury_cutoff_date"] = f.treasury_cutoff_date.astype("datetime64[us]")
