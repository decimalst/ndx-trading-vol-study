...
from src import issued_calibration_search as run


class IssuedCalibrationContractTests(unittest.TestCase):
    def setUp(self):
        self.p = copy.deepcopy(run.CONTRACT)

    def test_literal_contract(self):
        run.validate(self.p)
        self.assertEqual(self.p["comparisons"]["cumulative_hypotheses"], 131)
    ...
    def test_both_controls_required(self):
        ...
        self.assertEqual(run.candidate_leads(rows), ["calibrated"])
    ...
    def test_failure_retains_two_p1(self):
        for error in [ValueError("INSUFFICIENT_DATA: support"), ValueError("optimizer")]:
            ...
            self.assertEqual(m["cumulative_hypothesis_count"], 131)
            self.assertEqual(
                [(x["candidate"], x["control"], x["p_conservative"]) for x in m["rows"]],
                [("calibrated", "baseline", 1.0), ("calibrated", "recent_frequency", 1.0)],
            )
    ...
    def test_no_new_monthly_baseline_fit_allowed(self):
        for value in [1, True, -1]:
            with self.assertRaisesRegex(ValueError, "reuses all monthly"):
    ...
    def test_complete_inherited_family(self):
        ...
        self.assertEqual(len(prior), 129)
        self.assertEqual(
