"""Prewritten causal-pool publication guards and separately scaled literal intervals."""
...
def fixture():
    rows = []
    for number, control in enumerate(("frozen_baseline", "recent_frequency")):
        rows.append({
            "candidate": "pooled", "control": control, "score": "brier", "horizon": 1,
    ...
    metrics = {
        ...
        "cumulative_hypothesis_count": 121, "new_forecasts": 1200,
        "preserved_forecasts": 600, "combined_forecasts": 1800, "common_application_origins": 603,
        "common_scored_origins": 600, "new_monthly_fits": 0,
    }
    verification = {
        "status": "VERIFIED", "protocol_sha256": "a"*64,
        "forecast_reconstruction": {"forecasts_verified": 1800, "application_states_verified": 603,
                                    "new_forecasts_verified": 1200, "preserved_forecasts_verified": 600,
                                    "new_monthly_fits": 0, "common_scored_origins": 600,
...
class PlotContracts(unittest.TestCase):
    ...
    def test_verified_current_identity_success_family_and_reconstruction_counts_required(self):
        for mode in ("status", "verification_hash", "metrics_hash", "failure", "aborted",
                     "family", "cumulative", "forecast", "fit", "origins", "proof_forecast", "proof_state", "proof_missing", "preserved", "combined", "applications"):
            with self.subTest(mode=mode):
                ...
                elif mode in ("family", "cumulative", "forecast", "fit", "origins", "preserved", "combined", "applications"):
                    key = {"family": "hypothesis_count", "cumulative": "cumulative_hypothesis_count",
                           "forecast": "new_forecasts", "fit": "new_monthly_fits", "origins": "common_scored_origins",
                           "preserved": "preserved_forecasts", "combined": "combined_forecasts",
                           "applications": "common_application_origins"}[mode]
                    metrics[key] += 1
                elif mode == "proof_missing":
                    verification.pop("forecast_reconstruction")
                else:
                    key = "forecasts_verified" if mode == "proof_forecast" else "application_states_verified"
    ...
    def test_counts_are_validated_without_fixed_empirical_expectations(self):
        metrics, verification = fixture()
        metrics.update(new_forecasts=1202, preserved_forecasts=601, combined_forecasts=1803,
                       common_scored_origins=601, common_application_origins=604)
        verification["forecast_reconstruction"].update(forecasts_verified=1803, application_states_verified=604,
                                                       new_forecasts_verified=1202, preserved_forecasts_verified=601,
                                                       common_scored_origins=601, common_application_origins=604)
        for row in metrics["rows"]:
            row["phases"][0]["n"] += 1
        plot.validated_rows(metrics, verification, "a"*64)
        for key in ("new_forecasts", "preserved_forecasts", "combined_forecasts",
    ...
    def test_invalid_input_creates_no_artifacts_and_main_failure_blocks_rendering(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root/"causal_pool.yaml").write_bytes(payload)
    ...
    def test_every_independent_cohort_count_matches_metrics(self):
        for key in ("new_forecasts_verified", "preserved_forecasts_verified", "new_monthly_fits",
                    "common_scored_origins", "common_application_origins"):
            metrics, verification = fixture()
            verification["forecast_reconstruction"][key] += 1
            with self.assertRaises(ValueError):
    ...
    def test_zero_fits_are_required_and_application_cohort_cannot_shrink(self):
        metrics, verification = fixture()
        for value in (True, False, 1, -1, float("nan")):
            bad = deepcopy(metrics)
            bad["new_monthly_fits"] = value
            with self.assertRaises(ValueError):
                plot.validated_rows(bad, verification, "a"*64)
        metrics["common_application_origins"] = 599
        verification["forecast_reconstruction"]["application_states_verified"] = 599
        with self.assertRaises(ValueError):
            plot.validated_rows(metrics, verification, "a"*64)
    ...
    def test_separate_scales_preserve_small_comparison_and_no_single_control_lead(self):
        metrics, verification = fixture()
        for row in metrics["rows"]:
            wide = row["control"] == "recent_frequency"
            row["lead"] = not wide
            row["p_holm_wave"] = 1e-8 if not wide else .9
            row["p_holm_cumulative"] = 1e-8 if not wide else .9
            for phase in row["phases"]:
                phase.update(delta=-.08 if wide else -.0006,
                             candidate_loss=.12 if wide else .1994, control_loss=.2,
                             ci95_envelope=[-.11, -.05] if wide else [-.0008, -.0004])
