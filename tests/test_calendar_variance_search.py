...
from src.calendar_variance_features import MODELS
...
def passing_row(control="baseline"):
    return {
        ...
        "phases": [
            {"name": "development", "n": 200, "delta": -0.006, "stability": []},
            {
                "name": "evaluation",
                "n": 300,
                "delta": -0.006,
            ...
            pd.DataFrame(
                {
                    ...
                    "y": 0.0001,
                    "prediction": 0.0001 if name == "calendar" else 0.00015,
                    "fit_origin": dates,
...
class CalendarSearch(unittest.TestCase):
    def test_literal_protocol(self):
        study.validate(protocol())
        self.assertEqual(study.WAVE_ALPHA, 0.05 / (8 * 9))
    ...
    def test_core_contract_amendments_rejected(self):
        ...
        for group, key, value in [
            ...
            ("inference", "seed", 1),
            ("comparisons", "cumulative_hypotheses", 107),
    ...
    def test_all_gates_and_both_controls_required(self):
        ...
        self.assertEqual(study.candidate_leads([a, b]), ["calendar"])
    ...
    def test_failed_wave_retains_both_as_p_one(self):
        for error, status in [
            ...
            self.assertEqual(
                (out["hypothesis_count"], out["cumulative_hypothesis_count"]), (2, 108)
            )
    ...
    def test_inherited_count_is_full_enumerated_family(self):
        ...
        self.assertEqual(
            sum(r["source"] == "reports/tail_shape/metrics.json" for r in rows), 3
        )

    def test_common_paired_metadata_and_positive_forecasts_required(self):
        original, dates = panel()
        for fault in ["missing", "duplicate", "target", "negative", "metadata"]:
            data = original.copy()
            ix = data.index[data.model.eq("calendar")][0]
            if fault == "missing":
                data = data.drop(ix)
                ...
                data.loc[ix, "prediction"] = -1
            else:
                data.loc[ix, "train_n"] = 1001
            with self.subTest(fault=fault), self.assertRaises(ValueError):
                study.evaluate(data, dates, protocol())

    def test_all_four_phase_scores_and_108_adjustment_preserved(self):
        data, dates = panel()
        ...
        self.assertEqual(result["cumulative_hypothesis_count"], 108)
