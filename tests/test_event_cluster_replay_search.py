"""Prewritten technical replay family, exact accounting and unchanged statistical gates."""
...
from src import event_cluster_replay_search as run


class EventClusterReplayContractTests(unittest.TestCase):
    def setUp(self):
        self.p = copy.deepcopy(run.CONTRACT)

    def test_literal_contract(self):
        run.validate(self.p)
        self.assertEqual(self.p["comparisons"]["cumulative_hypotheses"], 137)
    ...
    def test_failure_retains_three_p1(self):
        for error in [ValueError("INSUFFICIENT_DATA: support"), ValueError("optimizer")]:
            ...
            self.assertEqual(m["cumulative_hypothesis_count"], 137)
            self.assertEqual(
    ...
    def test_positive_literal_retained_schedule_count_required(self):
    ...
    def test_complete_inherited_family(self):
        ...
        self.assertEqual(len(prior), 134)
        self.assertEqual(
        ...
        failed = [x for x in prior if x["source"] == "reports/event_cluster/metrics.json"]
        self.assertEqual(len(failed), 3)
        self.assertTrue(all(x["p_conservative"] == 1 for x in failed))

    def test_stricter_wave21_allocation_is_used(self):
        self.assertEqual(run.WAVE_ALPHA, 0.05 / (21 * 22))
        row = self.row()
        row["p_holm_wave"] = 0.00011
        self.assertFalse(run.passes(row))

    def test_replay_generates_zero_forecasts_or_producer_fits(self):
        from tests.test_verify_event_cluster import inference_fixture

        panel, oldp, _, prior = inference_fixture()
        p = copy.deepcopy(self.p)
        p["original_index"] = oldp["original_index"]
        p["inference"]["bootstrap_draws"] = 199
        prior += [dict(prior[-1], candidate="failed_" + str(i)) for i in range(3)]
        calendar = __import__("pandas").bdate_range("2010-01-04", periods=421)
        result = run.evaluate(panel, calendar, p, 21, prior=prior)
        n = panel.origin.nunique()
        self.assertEqual(result["newly_generated_forecasts"], 0)
        self.assertEqual(result["new_producer_fits"], 0)
        self.assertEqual(result["retained_candidate_scored_forecasts"], 2 * n)
        self.assertEqual(result["original_control_forecasts"], 2 * n)
        self.assertEqual(result["combined_retained_forecasts"], 4 * n)
        self.assertEqual(result["retained_monthly_schedules"], 21)
        self.assertEqual(result["independent_stage_fits_verified"], 42)
        self.assertEqual(result["cumulative_hypothesis_count"], 137)
        self.assertNotIn("new_forecasts", result)
        self.assertNotIn("new_monthly_fits", result)
        self.assertEqual([x["study"] for x in result["rows"]], ["event_cluster_replay"] * 3)
