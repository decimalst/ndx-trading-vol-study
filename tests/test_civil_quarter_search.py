...
class CivilQuarterSearch(unittest.TestCase):
    def test_fixed_contract_and_family(self):
        study.validate(protocol())
        self.assertEqual(study.WAVE_ALPHA, 0.05 / (15 * 16))
        self.assertEqual(
            study.CONTRASTS,
            (
        ...
        self.assertEqual(len(study.inherited(protocol())), 121)
    ...
    def test_all_unevaluable_hypotheses_retained(self):
        ...
        self.assertEqual((m["hypothesis_count"], m["cumulative_hypothesis_count"]), (2, 123))
