"""Prewritten range-location publication guards and separately scaled literal intervals."""
...
from src import plot_range_alert as plot
...
class PlotContracts(unittest.TestCase):
    def test_replay_threshold_and_cumulative_family_are_not_previous_wave(self):
        self.assertEqual(plot.WAVE_ALPHA, 0.05 / (18 * 19))
        self.assertEqual(plot.CUMULATIVE_FAMILY, 129)
        protocol = yaml.safe_load((plot.ROOT / "range_alert.yaml").read_bytes())
        ...
        with (
            ...
            self.assertIn("Holm129", text)
            self.assertIn("closing location", text)
    ...
    def test_invalid_input_creates_no_artifacts_and_main_failure_blocks_rendering(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            folder = root / "reports/range_alert"
            folder.mkdir(parents=True)
            payload = b"synthetic protocol\n"
            (root / "range_alert.yaml").write_bytes(payload)
            ...
            verification["verified_output_hashes"] = {
                "reports/range_alert/metrics.json": hashlib.sha256(
