...
from src import civil_quarter_search as study


class CivilQuarterPublication(unittest.TestCase):
    def test_protocol_changed_during_pretests_prevents_registration(self):
        with tempfile.TemporaryDirectory() as tmp, ExitStack() as stack:
            root = Path(tmp)
            p = root / "civil_quarter.yaml"
            p.write_text("wave: 15\n")

            def checks(*args, **kwargs):
                p.write_text("wave: 16\n")
    ...
    def test_inherited_json_pin_is_checked_before_decode(self):
        with tempfile.TemporaryDirectory() as tmp, patch.object(study, "ROOT", Path(tmp)):
            ...
            rows = [
                {"candidate": str(i), "horizon": 1, "p_conservative": 0.5} for i in range(121)
            ]
