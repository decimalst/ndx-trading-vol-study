"""Prewritten issued-calibration publication guards and separately scaled literal intervals."""
...
from src import plot_issued_calibration as plot


def fixture():
    rows = []
    for number, control in enumerate(("baseline", "recent_frequency")):
        rows.append(
            {
    ...
    metrics = {
        ...
        "new_forecasts": 600,
        "reused_control_forecasts": 1200,
        "combined_forecasts": 1800,
...
class PlotContracts(unittest.TestCase):
    def test_replay_threshold_and_cumulative_family_are_not_previous_wave(self):
        self.assertEqual(plot.WAVE_ALPHA, 0.05 / (19 * 20))
        self.assertEqual(plot.CUMULATIVE_FAMILY, 131)
        protocol = yaml.safe_load((plot.ROOT / "issued_calibration.yaml").read_bytes())
        self.assertEqual(plot.WAVE_ALPHA, protocol["wave_alpha"])
        self.assertEqual(
            plot.CUMULATIVE_FAMILY, protocol["comparisons"]["cumulative_hypotheses"]
        )
        self.assertEqual(protocol["inference"]["bootstrap_draws"], 199999)
        minimum_p = 1 / (protocol["inference"]["bootstrap_draws"] + 1)
        self.assertLess(minimum_p, plot.WAVE_ALPHA / 2)
        self.assertLess(minimum_p, 0.1 * plot.WAVE_ALPHA / 2)
        ...
        with (
            ...
            self.assertIn("Holm131", text)
            self.assertIn("forecast-error memory", text)
    ...
    def test_counts_are_validated_without_fixed_empirical_expectations(self):
        ...
        metrics.update(
            new_forecasts=601,
            reused_control_forecasts=1202,
            combined_forecasts=1803,
            new_monthly_fits=0,
    ...
    def test_invalid_input_creates_no_artifacts_and_main_failure_blocks_rendering(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root / "issued_calibration.yaml").write_bytes(payload)
            digest = hashlib.sha256(payload).hexdigest()
            metrics["protocol_sha256"] = verification["protocol_sha256"] = digest
            verification["status"] = "VERIFIED"
            (folder / "metrics.json").write_text(json.dumps(metrics))
            verification["verified_output_hashes"] = {
                "reports/issued_calibration/metrics.json": hashlib.sha256(
            ...
            with patch.object(plot, "render_verified", return_value={}) as render:
                ...
                (folder / "metrics.json").write_bytes(original_metrics + b" ")
                render.reset_mock()
                with self.assertRaisesRegex(ValueError, "snapshot"):
                    plot.main(root)
                render.assert_not_called()
                (folder / "metrics.json").write_bytes(original_metrics)
    ...
    def test_application_cohort_cannot_shrink_and_bounded_brier_scores_are_valid(self):
        metrics, verification = fixture()
        for row in metrics["rows"]:
        ...
        for row in metrics["rows"]:
            ...
            for phase in row["phases"]:
                phase.update(
                    ...
                    candidate_loss=0.1 if wide else 0.894,
                    control_loss=0.9,
    ...
    def test_literal_intervals_zero_threshold_and_readable_render(self):
        ...
        with (
            ...
            for ax, row in zip(fig.axes, expected, strict=True):
                ...
                self.assertTrue(
                    any(
                        np.array_equal(line.get_xdata(), [-0.0005, -0.0005])
