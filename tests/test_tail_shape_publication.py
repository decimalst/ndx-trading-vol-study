"""Pre-fit publication failures preserve the whole three-contrast shape family."""
import hashlib
import json
import tempfile
import unittest
...
class TestTailShapePublication(unittest.TestCase):
    def _assert_failed_publication(self, fault):
        # All measurements, support counts, fits and scores are synthetic stubs.
        # Only persistence and failure transitions run, in a temporary tree.
        scored = {
            "rows": [
                {"study": "tail_shape", "candidate": candidate,
        ...
        support_audit = {"synthetic": True, "diagnostic": "preserve support audit"}
        ...
        with tempfile.TemporaryDirectory() as folder, ExitStack() as stack:
            ...
            protocol.write_text(json.dumps({"sources": {
                "daily": "daily.parquet", "vix": "vix.csv", "vix9d": "vix9d.csv",
                "vvix": "vvix.csv", "skew": "skew.csv", "skew_source": "skew.json",
                "skew_derived": "skew.parquet"}, "index": {}}))
            ...
            replacements = {
                ...
                "support_summary": lambda *_args: support_audit,
            ...
            stack.enter_context(patch.object(study.tf, "load_sources", return_value=(None, None, source_audit)))
            stack.enter_context(patch.object(study.tf, "build_features", return_value=(
            ...
            self.assertEqual((failed["hypothesis_count"], failed["cumulative_hypothesis_count"]), (3, 106))
            ...
            if fault == "registered_ledger":
                ...
                self.assertFalse((output/"support_audit.json").exists())
            else:
                ...
                self.assertEqual(json.loads((output/"support_audit.json").read_text()), support_audit)
            ...
            self.assertEqual(sum(r["event"] == "inherited" for r in events), 103)
            self.assertEqual(sum(r["event"] == "registered" for r in events), 1 if fault == "registered_ledger" else 3)
            self.assertEqual(sum(r["event"] == "evaluated" for r in events), int(fault == "evaluated_ledger"))
            self.assertTrue(all(r["event"] == "unevaluable" and r["p_conservative"] == 1 for r in events[-3:]))
            self.assertEqual({(r["candidate"], r["control"], r["score"]) for r in events[-3:]},
                             set(study.CONTRASTS))

    def test_report_failure_invalidates_all_three_and_preserves_diagnostics(self):
        self._assert_failed_publication("report")

    def test_partial_evaluated_ledger_failure_invalidates_all_three_and_preserves_diagnostics(self):
        self._assert_failed_publication("evaluated_ledger")

    def test_partial_registration_failure_still_accounts_for_all_three(self):
